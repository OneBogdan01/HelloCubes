...
void* Input::ProcessMouseThread(void* arg)
{
	...
	if (fmouse != NULL)
	{
		while (((Input*)arg)->mQuit == false) // so as long as mQuit is FALSE, this will endlessly loop
		{
			...
			((Input*)arg)->TheMouse.PositionX += (mousex / 1.0f);
			...
			((Input*)arg)->TheMouse.LeftButton = (b[0] & 1) > 0;
...
bool Input::Init()
{
	...
	// we send the Input class (this) as an argument to allow for easy cast ((Input*)arg)-> type access to the classes data. 
